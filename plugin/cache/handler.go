	...
	if i != nil && ttl > 0 {
		...
		if c.prefetch > 0 {
			...
			if i.Freq.Hits() >= c.prefetch && ttl <= threshold {
				// We used to start the prefetch in a goroutine, but because the responsewriter is closed
				// by coredns when this handler returns, this leads to a crash when any other plugin
				// accesses the (now partly closed) response writer.

				cachePrefetches.WithLabelValues(server).Inc()
				// When prefetching we loose the item i, and with it the frequency that we've gathered sofar.
				// See we copy the frequencies info back into the new item that was stored in the cache.
				prr := &ResponseWriter{ResponseWriter: w, Cache: c, prefetch: true, state: state, server: server}
