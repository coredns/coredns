...
func (s *testSession) writeMsg(tb testing.TB, m packer) (err error) {
	tb.Helper()

	_, err = s.Write(packMsg(tb, m))
	return err
}

// assertWriteMsgReceived asserts that server writes given message and client receives it.
func (s *testSession) assertWriteMsgReceived(tb testing.TB, m packer) {
	tb.Helper()
...
func setupPendingSession(tb testing.TB) *testSession {
	...
	sesh := setupSession(tb)
	go sesh.writeMsg(tb, establishingMsg) // remains pending until clientConn reads
...
func setupEstablishedSession(tb testing.TB) *testSession {
	...
	sesh := setupSession(tb)
	sesh.assertWriteMsgReceived(tb, establishingMsg)
...
func setupClosingSession(tb testing.TB) *testSession {
	...
	sesh.assertWriteMsgReceived(tb, closeMsg)
		...
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				...
				synctest.Test(t, func(t *testing.T) {
					...
					if tc.wantErr == nil {
						sesh.assertWriteMsgReceived(t, tc.m)
					} else {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				synctest.Test(t, func(t *testing.T) {
					...
					if tc.wantErr == nil {
						...
						defer wg.Wait()
						wg.Go(func() {
							sesh.writeMsg(t, tc.m)
						})
						sesh.assertClientReadMsg(t, establishingMsg)
						sesh.assertClientReadMsg(t, tc.m)
					} else {
						sesh.assertWriteMsgFails(t, tc.m, tc.wantErr)
					}
				})
...
func TestSessionCloseNotify(t *testing.T) {
	...
	t.Run(StatePending.String(), func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			...
			defer wg.Wait()
			wg.Go(func() {
				sesh.writeMsg(t, closeMsg)
			})
			sesh.assertClientReadMsg(t, establishingMsg)
			sesh.assertClientReadMsg(t, closeMsg)
	...
	t.Run(StateEstablished.String(), func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			sesh := setupEstablishedSession(t)
			sesh.assertWriteMsgReceived(t, closeMsg)
			sesh.assertState(t, StateClosing)
		})
	})
