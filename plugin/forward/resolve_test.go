...
func TestReResolveTTLExpired(t *testing.T) {
	...
	if resolveCount.Load() == initialResolveCount {
...
func TestReResolveTTLDedupWithMixedEntries(t *testing.T) {
	...
	f.proxyMu.RLock()
	proxyCount := len(f.proxies)
	proxyAddrs := make([]string, 0, len(f.proxies))
