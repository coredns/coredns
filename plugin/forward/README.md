...
* `failover` If the `RCODE` (i.e. `ServFail`、`Refused`) is returned by the remote then execute failover logic to try the next upstream. If no upstreams are available, the response from the last attempt is returned. It is used to fix the problem of a service in multiple upstreams that is exceptionally exceptional or denied access, and it is the first to do not try to process the request using another upstream
...
Because of the `policy` is `sequential`, The following would try `1.2.3.4`、`5.6.7.8`、`9.0.1.2` sequentially. If the response from `1.2.3.4` is `ServFail` or `Refused`, try 5.6.7.8. If the response from 5.6.7.8 is `ServFail` or `Refused`, try 9.0.1.2.
