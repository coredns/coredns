...
// AuthInfoFromConfig gets authentication-related fields from a kubeconfig file
func (k *Kubernetes) AuthInfoFromConfig(config Config, contextName string) {
	var contextUser, contextCluster string
	for _, context := range config.Contexts {
		if context.Name == contextName {
			contextUser = context.Context.AuthInfo
			contextCluster = context.Context.Cluster
		}
	}
	for _, cluster := range config.Clusters {
		if cluster.Name == contextCluster {
			k.APICertAuth = cluster.Cluster.CertificateAuthority
		}
	}
	for _, user := range config.AuthInfos {
		if user.Name == contextUser {
			k.APIClientKey = user.AuthInfo.ClientKey
			k.APIClientCert = user.AuthInfo.ClientCertificate
			k.APIUsername = user.AuthInfo.Username
			k.APIPassword = user.AuthInfo.Password
			k.APIToken = user.AuthInfo.Token
		}
	}
}
