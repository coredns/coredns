...
// RFC 4592 §2.2.1: a wildcard is the source of synthesis only for names whose
// closest encloser is the wildcard owner's parent. If a closer empty
// non-terminal exists between the wildcard's parent and the queried name, the
// shallower wildcard must not be expanded and the name is an NXDOMAIN.
