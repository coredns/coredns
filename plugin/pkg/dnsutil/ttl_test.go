...
func TestMinimalTTLNoSOA(t *testing.T) {
	m := new(dns.Msg)
	m.SetQuestion("bar.www.example.org.", dns.TypeA)
	m.Rcode = dns.RcodeSuccess
	m.Answer = []dns.RR{test.CNAME("bar.www.example.org. 3500 IN CNAME foo.example.org.")} // but we add a cname with the name!
...
func TestMinimalTTLNoErrorWithSOA(t *testing.T) {
	m := new(dns.Msg)
	m.SetQuestion("bar.www.example.org.", dns.TypeA)
	m.Rcode = dns.RcodeSuccess
