...
const (
	...
	errSqlSetupArgExp     = "argument for '%s' expected"
)

// Config holds the configuration for SQL Plugin
type Config struct {
	automigrate    bool
	dsn            string
	dsnFile        string
...
// setup is the function that gets called when the config parser see the token "sql". Setup is responsible
// for parsing any extra options the sql plugin may have.
	...
	for c.Next() {
		for c.NextBlock() {
			...
			case "dsn":
				...
				if len(args) <= 0 {
					return plugin.Error(plgName, fmt.Errorf(errSqlSetupArgExp, "dsn"))
				}
				cfg.dsn = args[0]
			case "file":
				args := c.RemainingArgs()
				if len(args) <= 0 {
					return plugin.Error(plgName, fmt.Errorf(errSqlSetupArgExp, "file"))
				}
				cfg.dsnFile = args[0]
			case "automigrate":
				var err error
				args := c.RemainingArgs()
				if len(args) <= 0 {
					return plugin.Error(plgName, fmt.Errorf(errSqlSetupArgExp, "automigrate"))
				}
			...
			case "debug":
				...
				args := c.RemainingArgs()
				if len(args) <= 0 {
					return plugin.Error(plgName, fmt.Errorf(errSqlSetupArgExp, "debug"))
				}
			...
			case "zone_update_duration":
				...
				args := c.RemainingArgs()
				if len(args) <= 0 {
					return plugin.Error(plgName, fmt.Errorf(errSqlSetupArgExp, "zone_update_duration"))
				}
