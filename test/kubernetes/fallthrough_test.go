...
func TestKubernetesFallthrough(t *testing.T) {
	...
	rmFunc, upstream, udp := testk8s.UpstreamServer(t, "example.net", testk8s.ExampleNet)
	defer upstream.Stop()
	defer rmFunc()
...
`
	err := testk8s.LoadCorefileAndZonefile(corefile, clusterLocal)
	if err != nil {
		t.Fatalf("Could not load corefile/zonefile: %s", err)
	}
	testk8s.DoIntegrationTests(t, dnsTestCasesFallthrough, "test-1")
}
